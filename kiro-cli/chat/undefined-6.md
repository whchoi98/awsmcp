...
*   Unintended system changes (예기치 않은 시스템 변경)

    요청이 의도와 다르게 해석되어 비정상적인 파일 수정 또는 시스템 변경이 발생할 수 있음.
*   AWS resource modifications (AWS 리소스 변경)

    자원 생성·수정·삭제가 예상치 못하게 수행되어 프로덕션 환경에 영향을 주거나 비용이 증가할 수 있음.
*   Data loss (데이터 손실)

    삭제 또는 덮어쓰기 명령이 실행될 경우 중요한 데이터가 복구 불가능한 상태로 손실될 수 있음.
*   Security vulnerabilities (보안 취약점 노출)

    사용자 승인 없이 실행된 명령이 시스템의 보안 상태를 악화시킬 가능성 존재.
...
### Restricting File Access(파일 접근 제한)
