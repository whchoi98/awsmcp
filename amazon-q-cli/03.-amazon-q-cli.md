...
또한, 글로벌 설정 디렉토리(Global Amazon Q CLI directory)는 항상 \~/.aws/amazonq 경로에 존재하며, 전역 설정 값(컨텍스트, 프로필, MCP 등)을 불러오는 데 사용됩니다.<br>

📋 정리하면:

* 전역 디렉토리: \~/.aws/amazonq → 전역 설정값 (프로필, 컨텍스트, MCP 등)을 불러옴
* 프로젝트 디렉토리: 현재 디렉토리의 .amazonq/ → 프로젝트별 설정 파일을 읽어옴<br>
...
### 🧪 Task 03: 실험적 기능(Experimental Features) 활성화<br>
