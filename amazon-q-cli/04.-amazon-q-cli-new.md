...
결과 예시:결과 예시:❯ q ai "list all my aws account"                                                                                                                                                   22:101
...
```bash
"Python Flask 애플리케이션을 실행하는 Dockerfile을 보여줘."
```
...
만약 여러분의 프로젝트 워크스페이스에 이러한 파일이 존재한다면, Amazon Q CLI는 이를 읽고 컨텍스트로 자동 추가합니다. 그리고 해당 규칙에 일치한 파일 숫자가 표시됩니다.
...
이전 실습에서 Amazon Q CLI 세션에 컨텍스트를 제공하는 “리소스(Resources)”에 대해 배웠습니다.

이제 우리는 해당 리소스를 직접 추가하거나 제거하면서, Amazon Q CLI가 어떤 정보를 참조할지를 더 정교하게 제어할 수 있어야 합니다.
...
* 파일을 열어 아무 텍스트나 입력하고 저장하세요. (내용은 중요하지 않으며, 임의의 텍스트로 충분합니다.)
* 해당 파일을 컨텍스트에 추가합니다:
...
👤 /Users/yourname/project-directory/project-standards.md (~590 tkns)
...
* Amazon Q CLI 세션 종료 및 재시작 후:

```
/context show
```

이때 project-standards.md는 사라져 있고, 기본 리소스만 남아있는 것을 확인할 수 있습니다.

이처럼 Amazon Q CLI는 컨텍스트를 세밀하게 제어할 수 있도록 도와주며, 작업의 정확도와 맥락 이해도를 높이기 위한 핵심 기능 중 하나입니다.

***

### 🧩 컨텍스트 관리 (Context Management)

앞서 살펴본 바와 같이, Amazon Q CLI는 최대 200,000 토큰(token)의 컨텍스트 윈도우(context window)를 제공합니다. 컨텍스트에 리소스를 추가할수록 이 토큰 용량을 점점 소모하게 되며, 이 한도를 초과하거나 가까워질 경우 출력 결과가 불안정해지거나 일관성이 떨어질 수 있습니다.
...
<table data-header-hidden><thead><tr><th width="155.1500244140625"></th><th></th></tr></thead><tbody><tr><td>항목</td><td>설명</td></tr><tr><td>Context files</td><td>.md 형식의 리소스 파일들이 사용하는 토큰 수</td></tr><tr><td>Tools</td><td>현재 세션에 활성화된 도구(Tools)들이 사용하는 토큰 수</td></tr><tr><td>Q responses</td><td>Amazon Q가 이전에 생성한 응답들의 누적 토큰 수</td></tr><tr><td>Your prompts</td><td>사용자가 입력한 프롬프트의 누적 토큰 수</td></tr></tbody></table>
...
Amazon Q CLI에서 컨텍스트 윈도우(context window)가 가득 차기 시작하면, “/compact” 명령어를 통해 현재 컨텍스트를 요약(compact)할 수 있습니다. 이 기능은 모든 대화 이력과 참조 리소스를 요약한 요약본(summary)을 생성한 뒤, 기존 컨텍스트를 해당 요약으로 대체합니다.

이렇게 하면 사용된 토큰 수를 줄이면서도 주요 내용은 유지할 수 있게 됩니다.

#### 📈 Compacting 전후의 사용량 비교 예시
...
#### ✅  Compact 요약 결과 예시 요약 구성

* Conversation Summary: 어떤 기능을 구현했는지 요약
* Tools Executed: 어떤 도구들이 실행되었는지
* Code Created: 생성된 클래스, 함수, 라우트 등
* Key Insights: 핵심 아이디어, 구현 전략\
...
Amazon Q CLI는 기본적으로 컨텍스트 한도에 가까워질 때 자동으로 compact를 시도합니다.

하지만 이 기능을 사용자 제어 하에 두고 싶다면 설정 파일에 다음과 같이 입력하세요:
...
앞선 실습들에서는 Context Engineering(컨텍스트 설계)에 대해 학습하고, Amazon Q CLI에서 컨텍스트를 효과적으로 관리하는 다양한 기술들을 살펴보았습니다.

이번 실습에서는 새로운 실험적 기능(experimental feature)인 Tangent Mode(탄젠트 모드)를 소개합니다.
...
```
이전에 나눈 대화 기록은 가지고 있지 않습니다.Amazon Q와의 각 채팅 세션은 이전 상호작용에 대한 기억 없이 새롭게 시작됩니다.
```
...
```
우리는 Python의 FastAPI 프레임워크에 대해 이야기하고 있었어요. 제가 설명했던 주요 특징으로는 고성능, 자동 문서화, 타입 안정성, 그리고 비동기 지원 등이 있었고,
간단한 코드 예제와 설치 방법도 함께 안내드렸습니다.
```
...
이제 세션이 완전히 초기화된 상태로 돌아가며, 컨텍스트에 저장되었던 대화 이력이나 리소스는 제거됩니다.
...
Amazon Q CLI를 자주 사용하다 보면, 특정 작업을 수행하기 위해 반복해서 동일한 지시사항을 입력하게 되는 경우가 많습니다. 이러한 반복적인 설정(repetitive setup)은 생산성을 떨어뜨릴 뿐만 아니라, 실수로 어떤 기준을 빠뜨릴 경우 출력 결과의 일관성(consistency)도 저하될 수 있습니다.
...
위 출력에서 볼 수 있듯이, Amazon Q CLI는 .amazonq/rules/\*\*/\*.md 경로의 마크다운 파일들을 자동으로 검색 및 적용합니다.  현재는 규칙 파일을 만들지 않았기 때문에, “No files matched”로 표시됩니다.

#### ✅ Rules 파일 공유

규칙 파일을 .amazonq/rules 디렉토리에 저장하면, 프로젝트 전체와 함께 버전 관리(Git 등)가 가능하므로, 다른 팀원이 코드를 클론하거나 체크아웃했을 때 자동으로 규칙이 적용됩니다.
...
Amazon Q CLI에서는  `!` 를 명령어 앞에 붙이면,해당 명령을 실제 셸에서 실행하고, 출력을 채팅창 내에서 반환해줍니다.
...
> 대화 내용을 .json 형식으로 현재 디렉토리에 저장합니다.
...
<table data-header-hidden><thead><tr><th width="240.54693603515625"></th><th></th></tr></thead><tbody><tr><td>기능</td><td>명령어 / 설명</td></tr><tr><td>셸 명령 실행</td><td>! &#x3C;명령어></td></tr><tr><td>현재 디렉토리 기준 세션 재개</td><td>q chat --resume</td></tr><tr><td>대화 저장</td><td>/save &#x3C;이름></td></tr><tr><td>대화 불러오기</td><td>/load &#x3C;이름></td></tr><tr><td>주의사항</td><td>명령어 출력이 컨텍스트를 차지하므로 사용량 관리 필요</td></tr></tbody></table>

이러한 기능을 통해 Amazon Q CLI는 단순한 채팅 도구가 아니라, 지속 가능한 개발 파트너처럼 대화를 저장하고 이어갈 수 있는 환경을 제공합니다.
