...
## 🧩 01. Command Line Completion – 기능 설명 및 Task 01
...
> “내가 지금 하려는 작업을 미리 알고, 적절한 명령어를 제시해주는 스마트한 CLI 비서” 처럼 작동합니다.
...
방향키(↑/↓)로 선택 후 ENTER 키를 눌러 원하는 작업을 수행하세요.
...
2. 출력된 명령어 및 옵션 중에서 원하는 항목을 선택해 실행해 보세요.

* ⚡ Execute → 바로 실행
* 📝 Edit → 수정 후 실행
* 🔄 Regenerate → 새로운 명령어 다시 생성

3. docker, git, aws, kubectl 같은 CLI 도구에 대해 다양한 시도를 해보세요:

* 예: q ai "중인 모든 Docker 컨테이너를 확인"
* 예: q ai "현재 브랜치를 origin으로 푸시"
* 예: q ai "kube-system 네임스페이스에 있는 모든 파드를 조회해줘."
...
```
/context add --global shared-guidelines.m
```
...
→ python-dev 프로필이 활성화된 상태로 Amazon Q CLI가 실행됩니다.\
...
아래는 Amazon Q CLI에서 프로필별 Context 파일을 추가/관리하는 방법 (Task 11\~12) 을 한국어로 번역하고 단계별로 정리한 안내입니다.
...
아래는 Amazon Q CLI의 Compacting 기능과 예시 결과(Task)를 한국어로 정리한 내용입니다. 이 기능은 context 용량이 커질 때 대화를 요약하고 압축하여 리소스를 절약하는 데 유용합니다.
...
아래는 Amazon Q CLI의 컨텍스트 초기화(Clearing) 기능에 대한 설명과 명령어 예시입니다.
...
아래는 Amazon Q CLI의 Tools 및 Trust 설정에 대한 설명과 Task 15 실습 내용을 한국어로 정리한 것입니다.
...
* 도구의 신뢰 여부는 Q CLI의 보안과 자동화 수준을 조절하는 중요한 요소입니다.
* 실습을 통해 도구의 권한 상태를 수시로 조정할 수 있음을 익히는 것이 목적입니다.
